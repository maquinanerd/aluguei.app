...
Implementação concreta (Fase 09): `IMetaAdsProvider` em `packages/integrations/src/meta-ads/` com `FakeMetaAdsProvider` determinístico (dry_run) e `MetaGraphAdsProvider` (Graph API v25.0, fetch nativo + zod, erros tipados, timeout e retry classificado) implementado e testado com mock, mas NÃO ativado no registry (IMPLEMENTED_NOT_LIVE_VERIFIED — ver `docs/integrations/META_ADS_HOMOLOGATION.md`). Token por conexão criptografado AES-256-GCM (ADR-028). Servidor MCP `apps/meta-mcp` (stdio) com 17 tools — ver `docs/META_MCP.md`.
