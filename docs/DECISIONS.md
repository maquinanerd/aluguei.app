...
- `POST /portal/auth/logout` revoga a linha de `portal_sessions` da sessão atual (`PortalAuth.sessionId`). Outras sessões da mesma pessoa continuam; revogar a concessão encerra todas.
