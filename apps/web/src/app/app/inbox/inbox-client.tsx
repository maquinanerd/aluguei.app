...
function InboxBody() {
  ...
  const msgQ = useQuery<{ messages: Message[]; total: number }>(
    selectedId ? `/conversations/${selectedId}/messages?limit=100` : null,
    [selectedId],
  );
  const intentQ = useQuery<{ intents: Intent[] }>(
  ...
  async function sendMessage(e: React.SyntheticEvent) {
    ...
    setSending(true);
    try {
      await apiClient(`/conversations/${selectedId}/messages`, {
        method: 'POST',
        ...
        <section className={cx('inbox-list', selectedId && 'inbox-list--hidden')}>
          <Stack gap={2} style={{ padding: 12, borderBottom: '1px solid var(--peg-border)' }}>
            ...
            <Select
              ...
              placeholder="Todas"
              options={Object.entries(CONVERSATION_STATUS_LABELS).map(([v, l]) => ({
                value: v,
                label: l,
              }))}
          ...
          <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
            {convQ.loading ? (
              ...
              <EmptyState
                title="Sem conversas"
                body="As conversas de WhatsApp aparecerão aqui."
                icon="messageCircle"
              />
            ) : (
              conversations.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Group between>
                      <Badge tone={CONVERSATION_STATUS_TONES[c.status] ?? 'neutral'}>
          ...
          {!selected ? (
            ...
            >
              <EmptyState
                ...
                body="Escolha uma conversa na lista para visualizar o histórico."
                icon="messageCircle"
              />
            </div>
          ) : (
            <>
              ...
              >
                <Group gap={2}>
                  ...
                  <Avatar
                    name={selectedParty?.name ?? selected.waContactId ?? '?'}
                    size="sm"
                    brand
                  />
                  <Stack gap={0}>
                    <span style={{ fontSize: 13, fontWeight: 600 }}>
                      {selectedParty?.name ?? selected.waContactId ?? 'Contato'}
                    </span>
                    <span className="peg-text-tertiary" style={{ fontSize: 11 }}>
                      {selected.channel}
                    </span>
                ...
                <Group gap={2}>
                  ...
                  <Badge tone={CONVERSATION_STATUS_TONES[selected.status] ?? 'neutral'}>
                    {label(CONVERSATION_STATUS_LABELS, selected.status)}
                  </Badge>
                </Group>
              </Group>
              <div
              ...
              >
                {msgQ.loading ? (
                  ...
                  [...msgQ.data.messages].reverse().map((m) => (
                    ...
                    >
                      <span style={{ fontSize: 13, lineHeight: '19px', whiteSpace: 'pre-wrap' }}>
                        {m.body}
                      </span>
                      <span
                  ...
                  <EmptyState
                    title="Sem mensagens"
                    body="Envie a primeira mensagem para este contato."
                    icon="messageCircle"
                  />
                )}
              </div>
              <form
                ...
                className="peg-group"
                style={{ gap: 8, padding: 12, borderTop: '1px solid var(--peg-border)' }}
              >
                <Input
                  size="md"
              ...
              <>
                <Stack gap={1}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 500 }}>
                    {selectedParty?.name ?? 'Sem contato vinculado'}
                  </span>
                  {selectedParty ? (
                    <span className="peg-text-tertiary" style={{ fontSize: 12 }}>
                      {selectedParty.type === 'COMPANY' ? 'Pessoa jurídica' : 'Pessoa física'}
                    </span>
                  ) : null}
                </Stack>
                <Stack gap={1}>
                  ...
                  {selectedLead ? (
                    <Badge tone={FUNNEL_TONES[selectedLead.status] ?? 'neutral'}>
                      {label(FUNNEL_LABELS, selectedLead.status)}
                    </Badge>
                  ) : (
                    <span className="peg-text-tertiary" style={{ fontSize: 12 }}>
                      Sem lead vinculado
                ...
                <Stack gap={1}>
                  ...
                  {intents.length === 0 ? (
                    <span className="peg-text-tertiary" style={{ fontSize: 12 }}>
                      Nenhuma intenção extraída ainda.
                    </span>
                  ) : (
                    intents.map((i) => (
                      <Stack
                      ...
                      >
                        ...
                        {i.confidence !== null ? (
                          <span className="peg-text-tertiary" style={{ fontSize: 11 }}>
                            confiança {Math.round(i.confidence * 100)}% · {i.extractedBy}
                          </span>
                        ) : null}
                      </Stack>
                    ))
