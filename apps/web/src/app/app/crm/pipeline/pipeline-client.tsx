    ...
    return (
      <div className="app-page">
        ...
        {loading ? (
          ...
          <EmptyState
            title="Nenhum lead"
            body="Crie um lead para começar o pipeline."
              ...
              return (
                ...
                >
                  <Group between style={{ padding: '8px 4px' }}>
                    <Group gap={2}>
                      <Badge tone={FUNNEL_TONES[stage] ?? 'neutral'}>
                  ...
                  <Stack gap={2}>
                    {items.map((lead) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          router.push(`/app/crm/leads/${lead.id}`);
                        }}
                        className="peg-card"
                        style={{
                          padding: 12,
                      ...
                      >
                        <span style={{ fontSize: 13, fontWeight: 500 }}>
                          {partyName.get(lead.partyId ?? '') ?? 'Sem contato'}
                        </span>
                        <span className="peg-text-tertiary" style={{ fontSize: 12 }}>
                        ...
                        {NEXT_STATUS[lead.status] ? (
                          ...
                          >
                            {busy === lead.id
                              ? 'Movendo…'
                              : `→ Avançar para ${label(FUNNEL_LABELS, NEXT_STATUS[lead.status])}`}
                          </span>
                        ) : null}
                      </button>
                    ))}
  ...
  const columns: Column<Lead>[] = [
    {
      ...
      header: 'Contato',
      render: (l) => (
        <span style={{ fontWeight: 500 }}>{partyName.get(l.partyId ?? '') ?? 'Sem contato'}</span>
      ),
    },
    {
      key: 'status',
    ...
    },
    {
      key: 'budget',
      header: 'Orçamento',
      render: (l) => (l.budgetMinCents !== null ? formatBRL(l.budgetMinCents) : '—'),
    },
    {
      key: 'actions',
      header: '',
