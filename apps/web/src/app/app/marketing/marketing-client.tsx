...
import {
  ...
  META_CAMPAIGN_STATUS_TONES,
  META_AD_PROFILE_STATUS_LABELS,
  META_AD_PROFILE_STATUS_TONES,
} from '@/lib/labels';
...
function MarketingBody() {
  ...
  const profilesQ = useQuery<{ adProfiles: AdProfile[]; total: number }>(
    '/meta/ad-profiles?limit=100',
    [],
  );
  const campaignsQ = useQuery<{ campaigns: Campaign[]; total: number }>(
  ...
  async function runAction(kind: string, id: string, title: string) {
    ...
    setBusyKey(key);
    try {
      await apiClient(`/meta/campaigns/${id}/${kind}`, {
        method: 'POST',
        body: { idempotencyKey: `${kind}-${id}-${String(Date.now())}` },
      });
  ...
  const activeProfiles = profiles.filter(
    (p) => p.status === 'PUBLISHED' || p.status === 'PREPARED',
  );
  ...
  return (
    <div className="app-page">
      ...
      {tab === 'visao-geral' ? (
        <div className="peg-grid cols-3">
          <Card title="Campanhas ativas" padless>
            <div style={{ padding: 16, fontSize: 22, fontWeight: 700 }}>
              {String(activeCampaigns.length)}
            </div>
            <div
              style={{ padding: '0 16px 16px', color: 'var(--peg-text-tertiary)', fontSize: 12 }}
            >
                ...
                >
                  <Stack gap={0} style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <span className="peg-text-tertiary" style={{ fontSize: 12 }}>
                      ...
                      {propertyMap.get(
                        profiles.find((p) => p.id === c.adProfileId)?.propertyId ?? '',
                      )?.title ?? '—'}
                    </span>
                  </Stack>
                  <Badge tone={META_CAMPAIGN_STATUS_TONES[c.status] ?? 'neutral'}>
                    {label(META_CAMPAIGN_STATUS_LABELS, c.status)}
                  </Badge>
                  <span style={{ fontSize: 13, fontWeight: 500 }}>
                    {formatBRL(c.dailyBudgetCents ?? c.lifetimeBudgetCents)}
                  </span>
                  <Group gap={1}>
                    {c.status === 'CREATED_PAUSED' ? (
                      <Button
                        size="xs"
                        variant="brand"
                        loading={busyKey === `publish:${c.id}`}
                        onClick={() => {
                          setConfirmAction({
                            kind: 'publish',
                            id: c.id,
                            title: `Publicar ${c.name}`,
                          });
                        }}
                      >
                        Publicar
                    ...
                    {c.status === 'PAUSED' ? (
                      <Button
                        size="xs"
                        variant="secondary"
                        loading={busyKey === `resume:${c.id}`}
                        onClick={() => {
                          setConfirmAction({
                            kind: 'resume',
                            id: c.id,
                            title: `Retomar ${c.name}`,
                          });
                        }}
                      >
                        Retomar
                ...
                >
                  ...
                  <Stack gap={0} style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <span className="peg-text-tertiary" style={{ fontSize: 12 }}>
                      ...
                      {listingMap.get(p.listingId ?? '')?.title ?? ''}
                    </span>
                  </Stack>
                  <Badge tone={META_AD_PROFILE_STATUS_TONES[p.status] ?? 'neutral'}>
                    {label(META_AD_PROFILE_STATUS_LABELS, p.status)}
                  </Badge>
                  <span className="peg-text-tertiary" style={{ fontSize: 12 }}>
                    {formatBRL(p.dailyBudgetCents ?? p.lifetimeBudgetCents)}
                  </span>
                </Group>
              ))}
            </Stack>
            ...
            {connection ? (
              <Group gap={2}>
                ...
                {connection.lastTestedAt ? (
                  <Badge tone="neutral">último teste {formatDate(connection.lastTestedAt)}</Badge>
                ) : null}
              </Group>
            ) : (
      ...
      {connQ.error || campaignsQ.error ? (
        <ErrorState
          body={connQ.error ?? campaignsQ.error ?? 'Falha ao carregar'}
          onRetry={() => {
            connQ.reload();
            campaignsQ.reload();
          }}
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (confirmAction)
            void runAction(confirmAction.kind, confirmAction.id, confirmAction.title);
        }}
